...
> 코드 구조 자체는 [CLAUDE.md](../CLAUDE.md)를 참고하고, 이 문서는 **프로젝트의 의도·결정 근거·로드맵·미결 질문**처럼 코드만 봐서는 알 수 없는 맥락을 담는다.
...
- **돈 쓴 응답은 절대 버리지 않는다.** 청구 후 단계(비용 계산) 실패는 `PaidResponseError`로 응답/토큰을 보존해 감사 로그에 남긴다.
