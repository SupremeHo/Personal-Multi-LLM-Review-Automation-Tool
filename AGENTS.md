...
`cli.ask` → `service_ask.ask` (mints ids, builds `LLMRequest`) → `service_ask.run_request` → `registry.get_provider` → `provider.ask` → `runner.run_chat` → returns `LLMCallResult` (or raises `PaidResponseError`) → `run_request` wraps it in an `LLMCallLog` → `service_ask.persist_log` → `storage_json.append_jsonl` writes to `_logs/<Provider>/<prefix>_response_log_<timestamp>.jsonl` → `storage_sqlite.import_jsonl_to_sqlite` upserts into `_db/llm_responses.db`. `compare` runs this per target under one `run_id`, collecting `ErrorInfo` for any failures.
...
- **Performanc**e: N+1 queries, checking unnecessary repeat statements
