...
Known gap (not yet fixed): unlike `OpenAI()` and `genai.Client()`, **`Anthropic()` does not raise when its key is missing** — it builds a keyless client, so `_default_client` is non-`None` and the "missing key disables the provider" contract above does not hold for Anthropic. The failure surfaces later as a 401 at call time, and `list-models` re-raises it as a traceback instead of the friendly message the other two get.
